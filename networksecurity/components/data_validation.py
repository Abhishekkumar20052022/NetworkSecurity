    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            if len(dataframe.columns) != len(expected_columns):
                logging.warning(f"Expected {len(expected_columns)} columns, got {len(dataframe.columns)}")
                return False
            return True
        except Exception as e:
            raise NetworkSecurityException(e, sys)
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not self.validate_number_of_columns(train_df):
                raise NetworkSecurityException("Train dataset does not contain expected number of columns", sys)
            if not self.validate_number_of_columns(test_df):
                raise NetworkSecurityException("Test dataset does not contain expected number of columns", sys)
